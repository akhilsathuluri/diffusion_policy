...
class PushTKeypointsEnvTwoAgents(PushTEnvTwoAgents):
    def __init__(
        self,
        legacy=False,
        block_cog=None,
        ...
        if agent_keypoints:
            ...
            Do += Dagentpos
        # obs + obs_mask
        # Basically can set some of the keypoints to 0 if needed which includes the agent position and hence the *2
        Dobs = Do * 2
        ...
        self.kp_manager = PymunkKeypointManagerTwoAgents(
    ...
    def _get_obs(self):
        ...
        if self.agent_keypoints:
            obj_map["agent1"] = self.agent1
            obj_map["agent2"] = self.agent2
        ...
        if self.agent_keypoints:
            # all agents are concatenated to one here when agent_keypoints is True
        ...
        if not self.agent_keypoints:
            ...
            # Set the number of masks per positions of agents here
            obs_mask = np.concatenate([obs_mask, np.ones((4,), dtype=bool)])
